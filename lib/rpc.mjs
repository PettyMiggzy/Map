...
// eth_getCode over the FREE RPC ($0 CU). Classifies each address as a contract or an EOA so
// liquidity pools / routers / staking contracts aren't mistaken for whale holders. Returns a
// Map(address -> 'contract' | 'eoa'); addresses that error out are simply omitted (retry later).
export async function classifyAddresses(addresses, { concurrency = 6, timeoutMs = 4000 } = {}) {
  const out = new Map();
  const queue = [...new Set(addresses.map((a) => String(a).toLowerCase()))];
  async function worker() {
    while (queue.length) {
      const a = queue.shift();
      try {
        const code = await rpc("eth_getCode", [a, "latest"], { allowFallback: false });
        out.set(a, code && code !== "0x" && code !== "0x0" ? "contract" : "eoa");
      } catch { /* leave unclassified; a later serve/cron retries */ }
    }
  }
