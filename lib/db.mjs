...
// The table self-creates on first use so no manual migration is needed to enable labeling.
let _labelsReady = null;
function ensureLabels() {
  if (!_labelsReady) {
    const db = requireDb();
    _labelsReady = db.query(
      `CREATE TABLE IF NOT EXISTS bm_labels (
         address text PRIMARY KEY, label text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now())`
    ).catch((e) => { _labelsReady = null; throw e; });
  }
  return _labelsReady;
}
