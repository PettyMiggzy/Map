...
export function attachFundingClusters(graph, funding = []) {
  ...
  // Funding edges connect holders funded by the same wallet. The funder is usually
  // NOT a holder (exchange/deployer), so a funder->wallet edge would have a phantom
  // endpoint and get dropped. Instead draw a star among the *holder siblings* — every
  // endpoint is a rendered node, so the cluster shows connected (bubblemaps-style).
  const pct = new Map(graph.nodes.map((n) => [n.id, n.pct || 0]));
  const fundingEdges = [];
  for (const [fn, ws] of byFunder) {
    if (ws.length < 2) continue;
    const funderIsHolder = holderIds.has(fn);
    if (funderIsHolder) {
      // funder is on-screen: star out from the funder itself
      for (const w of ws) if (w !== fn) fundingEdges.push({ from: fn, to: w, kind: "funding", funderIsHolder: true });
    } else {
      // funder off-screen: hub = biggest funded holder, spoke to every sibling
      const hub = ws.slice().sort((a, b) => (pct.get(b) || 0) - (pct.get(a) || 0))[0];
      for (const w of ws) if (w !== hub) fundingEdges.push({ from: hub, to: w, kind: "funding", funderIsHolder: false });
    }
  }
